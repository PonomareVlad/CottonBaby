...
const footerTemplate = page => `${dumpTemplates()}${liveReload()}<script>window.page=${JSON.stringify(page)}</script><script>window.cache=${db.exportCache()}</script>${footer}</body></html>`

const pageTemplate = ({url, setMeta}) => html`
    <app-page url="${url}" .setMeta="${setMeta}"></app-page>`

export async function createRenderThread(req, res, template = pageTemplate) {
